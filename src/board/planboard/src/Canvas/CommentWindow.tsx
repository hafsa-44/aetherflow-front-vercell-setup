...
type Props = {
  ...
  /**
   * @deprecated no longer used for tick status — presence is per-phase-room
   * and flickers when the other user switches phases, which made the old
   * "delivered" (double grey) state revert unpredictably. Status is now
   * purely based on the persisted `readBy` array: single tick (unread) or
   * double blue tick (read). Left in the prop type so Canvas.tsx doesn't
   * need to change what it passes down.
   */
  ...
  onTyping?: () => void;
  onStopTyping?: () => void;
  onCommentRead?: (commentId: string) => void;
};
...
// ── Status tick — sent (single grey) / read (double blue) ──────────────────────
// Only two states now, both derived from the persisted `readBy` array on the
// comment itself — nothing here depends on live presence, so it can't
// "revert" when the other person navigates to a different phase.
function StatusTick({ status }: { status: "sent" | "read" }) {
  if (status === "read") {
    return <span style={{ color: "#4c6ef5", fontSize: 11, letterSpacing: "-1px" }}>✓✓</span>;
  }
  return <span style={{ color: "#9aa3b8", fontSize: 11 }}>✓</span>;
}
...
const CommentModule: React.FC<Props> = ({
  ...
  }, [thread?.comments.length, typingUsers.length]);   // ← added typingUsers.length
  ...
  // Emits one comment:read per unread comment instead of writing the whole
  // commentThreads array through syncPatch — avoids the last-write-wins race
  // against a concurrently arriving board:patch (e.g. a new message).
  useEffect(() => {
    if (!activeThreadId || !thread || !currentUserId) return;
    thread.comments.forEach((c) => {
      const authorId = c.userId ?? c.user;
      if (authorId && authorId !== currentUserId && !(c.readBy ?? []).includes(currentUserId)) {
        onCommentRead?.(c.id);
      }
    });
  ...
  // sent → grey single check · read → blue double check.
  // Purely derived from the persisted readBy array — no presence involved,
  // so switching phases (or going offline entirely) can never make an
  // already-read message look unread again.
  const statusFor = (c: Comment): "sent" | "read" => {
      ...
      <div style={{
        ...
        {thread.comments.length === 0 ? (
          ...
          thread.comments.map((c: Comment, i: number) => {
            const isMe = c.userId ? c.userId === currentUserId : c.user === currentUser;
            return (
              <div
                key={i}
                className="cw-bubble"
                    ...
                    {isMe && (
                      <button
                        ...
                        style={{
                          ...
                          color: "#c7cbe0", fontSize: 12, cursor: "pointer",
