 ...
 *
 * Setup — no extra env vars needed. AI calls go through your Express backend.
 * Make sure GROQ_API_KEY is set in your server .env file.
 */
...
// All AI calls go to POST /api/ai/devboard/:projectId (authenticated).
...
// ── API call ───────────────────────────────────────────────────────────────
// Calls POST /api/ai/devboard/:projectId — the backend holds the Groq key
// and forwards the conversation. The key never reaches the browser bundle.
async function callDevboardAI(
  projectId: string,
  ...
  return (data as any).content ?? '(empty response)';
}
...
// ── Main component ─────────────────────────────────────────────────────────
interface AIPanelProps { projectId?: string; }
...
const AIPanel: React.FC<AIPanelProps> = ({ projectId }) => {
  ...
  const send = useCallback(async (userText: string) => {
    ...
    if (!projectId) {
      setError('No project context — open this board from inside a project.');
      return;
    }
    ...
    try {
      ...
      const reply = await callDevboardAI(projectId, history, ctrl.signal);
      setMessages(prev => [
        ...prev,
